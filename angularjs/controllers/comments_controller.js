...
function addComment(req, res, commentThread, currentThread, parentId, newComment){
	if (commentThread.id === parentId){
		commentThread.replies.push(newComment);
		updateCommentThread(req, res, commentThread);
	} else {
		for (var i = 0; i < currentThread.replies.length; i++) {
			var c = currentThread.replies[i];
			if (c._id == parentId){//c._id is object , can not use '==='
